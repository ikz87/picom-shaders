...
int color_rgb[] = { 255, 0, 255 }; // Color to replace, in rgb format 
float similarity = 0.4; // How many similar colors should be affected.
...
float base_opacity = 0.0; // Base opacity for matching pixels 
...
vec4 window_shader() {
        ...
        c.z >=chroma.z - similarity && c.z <=chroma.z + similarity)
    {
        ...
        else
        {
            ...
            {
                // Magic 
                c.w = base_opacity + avg_error*amount/similarity;
                c.x *= avg_error*amount/similarity;
                c.y *= avg_error*amount/similarity;
                c.z *= avg_error*amount/similarity;
